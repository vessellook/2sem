...
private:
    ...
    std::shared_ptr<my_namespace::SinglyLinkedList<std::shared_ptr<TreeNode>>> children_;
public:
    explicit TreeNode(Token token): token_(std::move(token)), parent_(nullptr), children_(new my_namespace::SinglyLinkedList<std::shared_ptr<TreeNode>>()) {}
    ...
    std::shared_ptr<my_namespace::SinglyLinkedList<std::shared_ptr<TreeNode>>> getChildren() { return children_; }
