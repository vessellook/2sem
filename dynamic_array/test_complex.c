...
void* add_2(void* element) {
    complex* i = malloc(sizeof(int));
    complex* j = (complex*) element;
    i->re = j->re + 2;
    i->im = j->im + 2;
    return (void*) i;
}
...
int main() {
    ...
    complex y1 = getElement_complex(a, 0);
    if(y1.re != x1.re || y1.im != x1.re) {
        printf("TEST FAILED: after 'setElement_complex()' 'getElement_complex()' return wrog value");
        scanf("%c", &chr); return 0;
    }
    setElement_complex(a, 1, x2);
    complex y2 = getElement_complex(a, 1);
    if(y2.re != x2.re || y2.im != x2.re) {
        printf("TEST FAILED: after 'setElement_complex()' 'getElement_complex()' return wrog value");
        scanf("%c", &chr); return 0;
    }
